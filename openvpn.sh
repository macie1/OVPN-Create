...
function installOpenVPN() {
	...
	if [[ ! -e /etc/openvpn/server/server.conf ]]; then
		if [[ $OS =~ (debian|ubuntu) ]]; then
			...
			make
			make install
			cp distro/systemd/openvpn-server@.service /etc/openvpn/server/
			sed -i -e "s/ProtectHome=true/ProtectHome=false/" /etc/openvpn/server/openvpn-server@.service
			mkdir /etc/openvpn
			mkdir /etc/openvpn/server
			cd /etc/openvpn/server/ || return
...
verb 3" >>/etc/openvpn/server/server.conf
	...
	if [[ $OS == 'arch' || $OS == 'fedora' || $OS == 'centos' ]]; then
		...
		sed -i 's|/etc/openvpn/server/server|/etc/openvpn/server|' /etc/systemd/system/openvpn-server@.service
		...
		# Another workaround to keep using /etc/openvpn/server/
		sed -i 's|/etc/openvpn/server/server|/etc/openvpn/server|' /etc/systemd/system/openvpn\@.service
